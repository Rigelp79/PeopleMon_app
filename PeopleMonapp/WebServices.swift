...
class WebServices: NSObject {
    ...
    //Store Auth token
    // use anytime you need it!! reuseable
    ...
    //Step 7a: function to check for authToken
    ...
    //Step 7b: function to check for authToken is expired
    ...
    // Step 7c: function to clear the authToken
    ...
    // AuthRouter - all network calls go through this. Look at request type and give it a network model. it creates the url request.
    enum AuthRouter: URLRequestConvertible {
        ...
        func asURLRequest() throws -> URLRequest {
            ...
            case .restRequest(let model):
                ...
                // Check for an authToken and if it exists, add it to the request
                if let token = WebServices.shared.authToken {
                    urlRequest.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
                }
                // Check for parameters and then add them to the url or the body depending on the Method
